...
describe("Plan 02-04: MEM-02 (record_observation) + MEM-04 (supersede) end-to-end", () => {
  ...
  it("tools/list snapshot includes record_observation + supersede AND the 23 v1 entries are byte-identical", async () => {
    ...
    // grows it to 27 (adds `get_outline`). Both record_observation
    // and supersede remain present byte-identical.
...
describe("Plan 02-05: MEM-03 recall end-to-end", () => {
  ...
  it("tools/list includes recall plus the 25 prior entries → 26 total", async () => {
    ...
    // Plan 03-02 adds `get_outline` (count 26 → 27); `recall` itself
    // remains present and the 25 prior entries are unchanged.
    expect(TOOLS).toHaveLength(27);
  });
...
describe("Plan 02-06: MCP Resources (MEM-09)", () => {
  ...
  it("tools/list count is 27 after Plan 03-02 (Resources still do NOT add tool entries)", async () => {
    const { TOOLS } = await import("./tool-registry.js");
    // Pin: MCP Resources (memory_stats, list_sinks) project from the
    // sink registry — they MUST NOT manifest as tools. Phase 3 plan
    // 03-02 added `get_outline` (26 → 27); the resource invariant
    // still holds at this new count.
    expect(TOOLS).toHaveLength(27);
