...
describe("ChunkId (Phase 5 / D-04)", () => {
  ...
  it("formatChunkId joins a DocId + 7-hex fragment", () => {
    expect(formatChunkId(docId, "a3f5b2c")).toBe("obsidian-fs://v/notes/foo.md#chunk-a3f5b2c");
  });
  ...
  it("parseChunkId rejects malformed strings", () => {
    ...
    expect(() => parseChunkId("obsidian-fs://v/p.md#chunk-XYZ1234")).toThrow(/Invalid ChunkId/); // upper-case hex
    expect(() => parseChunkId("obsidian-fs://v/p.md#chunk-toolong0")).toThrow(/Invalid ChunkId/);
  });
