...
| 2026-05-27 | [260527-j7h](.planning/quick/260527-j7h-create-contract-skill/SUMMARY.md) | Add `create-contract` agent skill (free-language intent→validated contract YAML) + auto-discovery of missing contracts | Skill-only, zero `src/` change. New `skills/create-contract/SKILL.md` (authoring + discovery + artifact-schema modes, references AUTHORING-CONTRACTS.md, honest about Phase-8.5-not-yet-real). Additive gap-logging in use-contracts → memory sink `_memory/_contract-gaps/`; clustered/suggested by create-contract --discovery (collect+suggest, not autonomous learning — true loops wait on ADR-029). Installer updated. 2 commits (aba05f4, c6e0f4a). `npm test` 1693 passed \| 11 skipped, 0 failures — zero regression. |
